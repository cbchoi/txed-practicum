...
def main():
    ...
    print(f"Starting Week 2 grading...")
    ...
    try:
        ...
        # 4. Week2AutoGrader 실행
        print("Running Week2AutoGrader...")
        grader_result = subprocess.run(
            ["java", "Week2AutoGrader", str(problem_dir)],
            cwd=grading_dir,
            capture_output=True,
            text=True,
            timeout=60
        ...
        if grader_result.returncode == 0:
            success_msg = f"Week 2 grading passed. Output: {grader_result.stdout[:200]}"
            print(success_msg)
            results_pass.write_text(f"PASS: {success_msg}")
            return True
        else:
            error_msg = f"Week 2 grading failed. Error: {grader_result.stderr[:200]}"
            print(error_msg)
            results_fail.write_text(f"FAIL: {error_msg}")
            return False
